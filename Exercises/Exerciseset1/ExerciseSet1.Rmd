...
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig.height=4,fig.width= 6, fig=TRUE}
round(100*sum(StockReturns[,2]),1)   # cumulative return
round(100*mean(StockReturns[,2]),3)  # avg daily return
round(100*sd(StockReturns[,2]),3)    # standard deviation
...
The code for downloading share prices is in the file 'dataSet1.R'. The code iterates through the list 'mytickers' and downloads price data for all the shares in the list.<br>
...
pf <- t(apply(transposedData, 2, mean))
names(pf) <- colnames(transposedData)
pnl_plot(pf)
...
1. Need to change the start date (startDate = "2000-12-28")
...
1. Evaluates to 60, ie. none of the entries in the matrices are equal. (there seems to be some precision/rounding error issue when writing to csv)
