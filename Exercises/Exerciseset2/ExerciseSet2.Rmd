...
**Your Answers here:**
<br>
1. Need 6 factors to explain at least 50% of variance.
2. The first component is heavily weighted towards interest rate factors (5/10 yr US notes, etc and secondly to foreign currency rates).
3. 10 first factors: interest rate (1,5), currency(2), equities (3,4), energy/oil (6), metals (7), soft commodities(8), precious metal (9), asian equities (10). These are macro factors.
4. Before the crisis a lower number of principal components are necessary to explain a given level of variance (scree plot curve is higher).
...
5. Looking at the scree plots before and during the crisis, we see that the first eigenvalues are higher during the crisis than before. This corresponds to there being some factors that explain a significantly larger portion of the variance during a crisis. This in term means that markets are higher correlated during crises. This implies that the assets that may be a good hedge in normal years will not be so in crises.
...
1. The first part creates a list of different windows that one may be interested in looking at (10-day, 30-day etc). It then creates the signal matrix which contains the sum of the moving averages over the various windows. It then normalizes the rows (i.e. each day) to add up to one (dollar). Finally it looks at the signal two days before to establish the position in order to compute the pnl.
2. The sequence determines the windows looked at. Could for instance change to only look at the past ten days.
